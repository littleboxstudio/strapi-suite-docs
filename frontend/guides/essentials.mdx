...
You can inject custom data into a template using a `content.ts` file. This file usually resides in your template folder (e.g., `app/templates/default/content.ts`). Check the [Templates](/frontend/guides/templates) section for a better understanding of how `content.ts` works.
...
The `i18n` directory configures the `next-intl` library. It handles the request scope and fetches the translation strings directly from Strapi.

**What are "Translations"?**

These translations are standalone strings that do not belong to any specific document (page) context. They are typically used for static UI elements like buttons ("Book now"), labels ("About us"), or error messages. By fetching them from the backend, you ensure these strings can be managed via the CMS without requiring code deployments for text changes.

For more details on how these are managed in the backend, refer to the [Translations](/backend/guides/translations) guide.
